...
def main():
    ...
    logger.info("="*70)
    logger.info("MS Teams Auto-Hide Script Started")
    ...
    if not os.path.exists(CHROME_PROFILE_PATH):
        ...
        logger.error("Please update CHROME_PROFILE_PATH in the script.")
        return
    
    logger.info(f"Using Chrome profile: {CHROME_PROFILE_PATH}")
    logger.warning("Please ensure Chrome is closed before continuing...")
    
    # Check if Chrome is running
    import subprocess
    try:
        result = subprocess.run(['powershell', '-Command', 'Get-Process chrome -ErrorAction SilentlyContinue | Measure-Object | Select-Object -ExpandProperty Count'], 
                              capture_output=True, text=True, timeout=5)
        chrome_count = result.stdout.strip()
        if chrome_count and int(chrome_count) > 0:
            logger.error(f"Chrome is still running ({chrome_count} processes detected)!")
            logger.error("Please close all Chrome windows and try again.")
            logger.info("Or run: Stop-Process -Name chrome -Force")
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info("This may take 10-20 seconds if Chrome is not already open...")
            ...
            try:
                ...
                logger.error("Make sure Chrome is completely closed.")
                logger.error("If Chrome is closed, try waiting 10 seconds and running again.")
                return
            ...
            try:
                page.goto(TEAMS_URL, wait_until="networkidle", timeout=60000)
